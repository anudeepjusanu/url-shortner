...
const StatsSection = () => {
  ...
  const stats = [
    {
      value: t("10M+", "10 مليون+"),
      label: t("Links shortened", "رابط مختصر"),
      note: t(
        "From agencies, shops, and developers",
        "من وكالات ومتاجر ومطورين",
      ),
    },
    {
      value: t("50K+", "50 ألف+"),
      label: t("Active users", "مستخدم نشط"),
      note: t(
        "In Riyadh, Jeddah, Dammam, and beyond",
        "في الرياض وجدة والدمام وأكثر",
      ),
    },
    {
      value: "99.9%",
      ...
      note: t(
        ...
        "خدمة موثوقة تقدر تعتمد عليها",
      ),
    },
    {
      value: t("<150ms", "<150 ميلي ثانية"),
      label: t("Redirect time", "متوسط سرعة التحويل"),
      note: t("Fastest in the region", "الأسرع في المنطقة"),
    },
  ];
  ...
  const testimonials = [
    {
      ...
      quote: t(
        `We used ${brand.name} for our Ramadan campaign and knew exactly which ad brought sales. We saved 40% of our ad budget.`,
        `استخدمنا ${brand.name} لحملة رمضان وعرفنا بالضبط أي إعلان جاب مبيعات. وفرنا 40% من ميزانية الإعلانات.`,
      ),
    },
    {
    ...
    <section className="py-16 bg-foreground">
      <div className="container mx-auto px-6">
        <p className="text-center text-background/50 font-body text-sm mb-8">
          {t(
            "Trusted by marketers and developers across Saudi Arabia",
            "يثق فينا مسوقين ومطورين من كل السعودية",
          )}
        </p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-16">
          {stats.map((stat, i) => (
            <div key={i} className="text-center">
              <p className="font-display text-3xl md:text-4xl font-bold text-background">
                {stat.value}
              </p>
              <p className="text-background/60 font-body text-sm mt-1">
                {stat.label}
              </p>
              <p className="text-background/40 font-body text-xs mt-0.5">
                {stat.note}
              </p>
            </div>
          ))}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {testimonials.map((testimonial, i) => (
            ...
            >
              ...
              <div>
                <p className="text-background font-display font-semibold text-sm">
                  {testimonial.name}
                </p>
