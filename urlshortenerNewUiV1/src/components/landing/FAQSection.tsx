...
const FAQSection = () => {
  ...
  const faqs = [
    {
      q: t(
        "Is my data stored inside Saudi Arabia?",
        "هل بياناتي محفوظة داخل السعودية؟",
      ),
      a: t(
    ...
    {
      q: t(
        "Can I use my own domain like go.mystore.sa?",
        "هل أقدر أستخدم نطاقي الخاص مثل go.متجري.sa؟",
      ),
      a: t(
    ...
    {
      q: t(
        `Is ${brand.domain} really free?`,
        `هل ${brand.domain} مجاني فعلاً؟`,
      ),
      a: t(
    ...
    {
      q: t(
        "What information shows up in analytics?",
        "ايش المعلومات اللي تظهر في التحليلات؟",
      ),
      a: t(
        ...
        <div className="grid lg:grid-cols-5 gap-12 lg:gap-16">
          ...
          >
            <h2 className="font-display text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[hsl(var(--navy))] mb-4">
              {t(
                "Questions before you start? Here are the answers",
                "أسئلة قبل ما تبدأ؟ هنا إجاباتها",
              )}
            </h2>
            <p className="font-body text-base text-[hsl(var(--navy))]/60 leading-relaxed">
              {t(
                `Everything you need to know about ${brand.domain}. Can't find an answer? Chat with us on WhatsApp.`,
                `كل اللي تحتاج تعرفه عن ${brand.domain}. ما لقيت جوابك؟ كلمنا على واتساب.`,
              )}
            </p>
          </motion.div>
            ...
            <Accordion type="single" collapsible className="w-full space-y-3">
              {faqs.map((faq, i) => (
                ...
                >
                  <AccordionItem
                    value={`faq-${i}`}
                    className="bg-white rounded-2xl border border-[hsl(var(--navy))]/10 px-6 shadow-soft"
                  >
