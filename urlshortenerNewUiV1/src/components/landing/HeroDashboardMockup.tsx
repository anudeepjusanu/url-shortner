    ...
    <div className="relative rounded-xl border border-border bg-card shadow-soft overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border bg-muted/50">
        ...
        <div className="flex-1 mx-8">
          <div
            className="bg-background border border-border rounded-md px-3 py-1 text-xs text-muted-foreground font-body max-w-xs"
            dir="ltr"
          >
      ...
      <div className="p-4 sm:p-6 md:p-8">
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-4">
            {[
              {
                label: t("Total clicks", "إجمالي النقرات"),
                value: fmtNum(12847),
              },
              {
                label: t("Unique visitors", "زوار فريدون"),
                value: fmtNum(8293),
              },
              ...
              <div
                key={stat.label}
                className="bg-background border border-border rounded-lg p-2.5 sm:p-4"
              >
                <p className="text-[10px] sm:text-xs text-muted-foreground font-body leading-tight">
                  {stat.label}
                </p>
                <p className="text-base sm:text-xl font-display font-bold text-foreground mt-1">
                  {stat.value}
                </p>
              </div>
            ))}
          ...
          <div className="bg-background border border-border rounded-lg p-5">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-display font-semibold text-foreground">
                {t("Clicks over time", "النقرات عبر الزمن")}
              </p>
              <span className="text-xs text-muted-foreground font-body">
                {t("Last 12 days", "آخر 12 يوماً")}
              </span>
            </div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={c.name} className="flex items-center gap-2">
                    <span className="text-[11px] font-body text-muted-foreground flex-1 whitespace-nowrap">
                      {c.name}
                    </span>
                    ...
                    <span className="text-[11px] font-display font-bold text-foreground w-7 text-end">
                      {fmtPct(c.pct)}
                    </span>
                  </div>
                ))}
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={h.name} className="flex items-center gap-2">
                    <span
                      className="text-[11px] font-body text-muted-foreground flex-1 whitespace-nowrap"
                      dir="ltr"
                    >
                      {h.name}
                    </span>
