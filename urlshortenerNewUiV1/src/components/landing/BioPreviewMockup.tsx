...
import {
  MessageCircle, Calendar, Instagram, Music, UtensilsCrossed,
  MapPin, Globe, BadgeCheck, Phone, ShoppingBag, Image as ImageIcon, Mail, Star, Sparkles,
  Youtube, Eye, Heart, Users,
} from "lucide-react";
...
type Social = "instagram" | "tiktok" | "whatsapp" | "twitter" | "globe" | "phone" | "youtube";

const SocialDot = ({ kind, tone = "light" }: { kind: Social; tone?: "light" | "dark" }) => {
  const Icon =
  ...
  return (
    <div className={`w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-sm ${
      ...
      <Icon className={`w-3.5 h-3.5 ${dark ? "text-gray-800" : "text-white"}`} />
    </div>
  );
};
  ...
  const personas: Persona[] = [
    {
      ...
      cardTone: "light",
      buttons: [
        { icon: ImageIcon, label: t("Latest Work", "أحدث الأعمال"), meta: t("New", "جديد"), accent: "hsl(15 85% 60%)" },
        { icon: Calendar, label: t("Book Consultation", "حجز استشارة"), accent: "hsl(280 55% 55%)" },
        { icon: Mail, label: t("Email Me", "راسلني"), accent: "hsl(200 80% 50%)" },
      ],
    ...
    },
    {
      ...
      cardTone: "light",
      buttons: [
        { icon: ShoppingBag, label: t("Order Online", "اطلب أونلاين"), meta: t("Free delivery", "توصيل مجاني"), accent: "hsl(35 80% 50%)" },
        { icon: UtensilsCrossed, label: t("View Menu", "تصفح القائمة"), accent: "hsl(15 70% 50%)" },
        { icon: MapPin, label: t("Get Directions", "الموقع"), accent: "hsl(200 70% 50%)" },
      ],
    ...
    },
    {
      ...
      cardTone: "light",
      buttons: [
        { icon: Sparkles, label: t("Case Studies", "دراسات الحالة"), meta: t("12 new", "12 جديدة"), accent: "hsl(195 85% 50%)" },
        { icon: MessageCircle, label: t("Contact Sales", "تواصل مع المبيعات"), accent: "hsl(150 60% 45%)" },
        { icon: Calendar, label: t("Free Consultation", "استشارة مجانية"), accent: "hsl(280 55% 55%)" },
      ],
              ...
              >
                ...
                <div className="relative z-10 flex-1 flex flex-col px-4 pt-16 pb-3">
                  ...
                  <div className="flex justify-center mb-2">
                    <div className="relative">
                      ...
                      <div className="absolute -bottom-0.5 -right-0.5 w-5 h-5 rounded-full bg-white flex items-center justify-center shadow">
                        <BadgeCheck className="w-4 h-4" style={{ color: p.accent }} fill={p.accent} stroke="white" strokeWidth={2.5} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-center mb-3">
                    <h3 className="text-[14px] font-bold text-white leading-tight drop-shadow">{p.name}</h3>
                    <p className="text-[10px] text-white/80 mt-0.5 drop-shadow">{p.title}</p>
                  </div>
                    ...
                    {p.buttons.map((b, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[11px] font-semibold text-white truncate flex-1 drop-shadow">{b.label}</span>
                          {b.meta && (
