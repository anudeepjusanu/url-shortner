...
import { Purpose, Industry, rankThemes } from "./StyleQuizStep";
...
const DesignStep = ({ draft, onUpdate, onContinue, quizResult }: Props) => {
  ...
  const suggestedThemes = hasQuiz ? ranked.slice(0, 6) : [];
  const [showAll, setShowAll] = useState(!hasQuiz);
  const themesToShow = showAll ? bioThemes : suggestedThemes;
  ...
  const setButtonColor = (color: string) => {
    ...
    onUpdate({ design: { ...draft.design, buttonColor: color, buttonTextColor: textColor } });
  };
  const setBackground = (bg: string) => onUpdate({ design: { ...draft.design, backgroundImage: bg } });
      ...
      <div>
        ...
        <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
          <h3 className="text-sm font-bold text-foreground">
            {showAll
              ? t("All themes", "كل التيمات")
              : t("Other suggestions for you", "اقتراحات أخرى لك")}
          </h3>
        ...
        <p className="text-xs text-muted-foreground mb-3">
          {t(
            "Layered on top of your background color.",
            "يُطبَّق فوق لون الخلفية الذي اخترته."
          )}
        </p>
        <div className="bg-card border border-border rounded-2xl p-4 shadow-soft mb-8">
          <div className="grid grid-cols-5 gap-2">
            {([
              ...
              const baseColor = draft.design.customColor || "#FFFBF5";
              const previewBg = ws.id === "solid"
                ? baseColor
                : buildWallpaper(ws.id, baseColor);
              return (
                <button
                  key={ws.id}
                  type="button"
            ...
            <div className="grid grid-cols-3 gap-2">
              {BUTTON_FILL_STYLES.map((s) => {
                ...
                return (
                  ...
                  >
                    <span className="px-3 py-1.5 rounded-md text-[11px] font-semibold w-full text-center" style={previewStyle}>
                      Aa
                    </span>
                ...
                return (
                  ...
                  >
                    <span
                      ...
                      style={{ background: baseColor, borderRadius: c.radius }}
                    />
                ...
                return (
                  ...
                  >
                    <span
                      ...
                      style={{ background: baseColor, boxShadow: s.css }}
                    />
                    {isAr ? s.label.ar : s.label.en}
                  </button>
                );
        ...
        <div className="bg-card border border-border rounded-2xl p-5 shadow-soft space-y-5">
          ...
          <div>
            ...
            <div className="inline-flex items-center gap-1 rounded-lg border border-border bg-background p-1">
              <button
                type="button"
                onClick={() => {
                  const next = Math.max(10, (draft.design.fontSize ?? 14) - 1);
                  onUpdate({ design: { ...draft.design, fontSize: next } });
                }}
              ...
              <button
                type="button"
                onClick={() => {
                  const next = Math.min(28, (draft.design.fontSize ?? 14) + 1);
                  onUpdate({ design: { ...draft.design, fontSize: next } });
                }}
                className="w-8 h-8 rounded-md hover:bg-muted text-foreground inline-flex items-center justify-center"
              >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                onUpdate({ design: { ...draft.design, fontFamily: e.target.value || undefined } })
              }
