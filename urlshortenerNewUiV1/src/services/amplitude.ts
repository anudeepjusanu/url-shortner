...
// Amplitude SDK is loaded dynamically so it never blocks FCP/LCP.
// All calls before the SDK resolves are queued and replayed.

type AmplitudeModule = typeof import("@amplitude/analytics-browser");

class AmplitudeService {
  private sdk: AmplitudeModule | null = null;
  private sdkPromise: Promise<AmplitudeModule> | null = null;
  private initialized = false;

  // Lazily import + init the SDK the first time it's needed
  private load(): Promise<AmplitudeModule> {
    if (this.sdkPromise) return this.sdkPromise;
    this.sdkPromise = import("@amplitude/analytics-browser").then((mod) => {
      this.sdk = mod;
      return mod;
    });
  ...
  // Called once from App — defers the heavy init until after first paint
  initialize(): void {
    ...
    // Use requestIdleCallback when available so init runs during browser idle time
    const run = () => {
      this.load().then((amplitude) => {
    ...
    if (typeof window !== "undefined") {
      if ("requestIdleCallback" in window) {
        (window as Window & { requestIdleCallback: (cb: () => void, opts?: { timeout: number }) => void }).requestIdleCallback(run, { timeout: 4000 });
      } else {
        // Fallback: wait until after load event, then give browser ~200ms head-start
        window.addEventListener("load", () => setTimeout(run, 200), { once: true });
      }
    }
  }

  private async exec(fn: (amplitude: AmplitudeModule) => void): Promise<void> {
    const amplitude = await this.load();
    fn(amplitude);
  }
