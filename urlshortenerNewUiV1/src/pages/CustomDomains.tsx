...
const CustomDomains = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      toast({
        title: t("Error", "خطأ"),
        description:
          error.message || t("Failed to delete domain", "فشل حذف الدومين"),
        variant: "destructive",
      });
    }
  };
    ...
    try {
      ...
      if (response?.success && response?.data?.verified) {
        ...
        toast({
          ...
          description: t(
            "DNS records not found yet. Please wait and try again.",
            "لم يتم العثور على سجلات DNS بعد. انتظر وحاول مرة أخرى.",
          ),
          variant: "destructive",
        });
      }
      ...
      toast({
        ...
        description:
          error.message ||
          t("Failed to verify domain", "فشل التحقق من الدومين"),
        variant: "destructive",
      });
    } finally {
      setVerifyingId(null);
  ...
  const handleSetDefault = async (id: string) => {
    ...
    try {
      await setDefaultDomain.mutateAsync(id);
    } catch (error: any) {
      toast({
        title: t("Error", "خطأ"),
        description:
          error.message ||
          t("Failed to set default domain", "فشل تعيين الدومين الافتراضي"),
        variant: "destructive",
      });
    } finally {
        ...
        <div className="text-center py-12">
          <p className="text-sm text-destructive font-body">
            {t(
              ...
              "فشل تحميل الدومينات. حاول مرة أخرى.",
            )}
          </p>
        </div>
      )}
      ...
      {!isLoading && !isError && domains.length === 0 && canEdit && (
        ...
        >
          ...
          <p className="text-sm text-muted-foreground font-body">
            {t(
              "No custom domains yet. Add your first domain!",
              "لا توجد دومينات مخصصة بعد. أضف دومينك الأول!",
            )}
          </p>
        </button>
      )}
      ...
      {!isLoading && !isError && domains.length > 0 && (
        <div className="space-y-3 sm:space-y-4">
          {domains.map((d) => (
            <div
              key={d._id}
              className="bg-background border border-border rounded-xl p-4 sm:p-5 hover:shadow-md transition-shadow"
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-3 sm:gap-4">
                  <div
                    className={`w-9 h-9 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center shrink-0 ${d.verificationStatus === "verified" ? "bg-primary/10" : "bg-muted"}`}
                  >
                    <Globe
                      className={`w-4 h-4 sm:w-5 sm:h-5 ${d.verificationStatus === "verified" ? "text-primary" : "text-muted-foreground"}`}
                    />
                  </div>
            ...
            <div className="space-y-3">
              <p className="text-[11px] sm:text-xs text-muted-foreground font-body">
                {dnsDialog.status === "verified"
                  ? t(
                      "Your domain is verified. Here are the DNS records for reference.",
                      "دومينك مُثبت. هذه سجلات DNS للاطلاع.",
                    )
                  : t(
                      "Add these records at your domain registrar. DNS propagation can take up to 72 hours.",
                      "أضف هذه السجلات عند مزود الدومين. قد يستغرق التحديث حتى 72 ساعة.",
                    )}
              </p>
                ...
                {getDnsRecords(dnsDialog).map((record) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <button
                        onClick={() =>
                          handleCopy(record.value, record.type + record.name)
                        }
          ...
          {dnsDialog && showVerified && (
            <div className="text-center py-4">
              ...
              <h3 className="font-display font-bold text-foreground text-base sm:text-lg mb-2">
                {t("Domain Verified!", "تم التحقق من الدومين!")}
              </h3>
              ...
              <p className="text-xs sm:text-sm text-muted-foreground font-body mb-6 max-w-md mx-auto">
                {t(
                  "Your domain has been verified successfully. You can now use it for short links.",
                  "تم التحقق من دومينك بنجاح. يمكنك الآن استخدامه للروابط المختصرة.",
                )}
              </p>
              <Button
                variant="outline"
