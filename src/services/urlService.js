...
class UrlService {
  ...
  async fetchUrlMetadata(url) {
    try {
      const response = await axios.get(url, {
        ...
        headers: {
          "User-Agent":
            "Mozilla/5.0 (compatible; URL-Shortener/1.0; +https://example.com/bot)",
        },
      });
      ...
      logger.info(`Failed to fetch metadata for ${url}:`, error.message);
      return {};
    }
  }
  ...
  async getUrlByShortCode(shortCode) {
    try {
      ...
      logger.error("Error fetching URL by short code:", error);
      return null;
    }
  }
  ...
  async getUrlStats(userId, organizationId = null) {
    try {
      ...
      const [totalUrls, activeUrls, totalClicks, uniqueClicks, topUrls] =
        await Promise.all([
          Url.countDocuments(filter),
