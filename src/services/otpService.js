...
const AUTHENTICA_KEY =
  process.env.AUTHENTICA_KEY ||
  "$2y$10$/JIkKbANcOor4YMkpEUDhu3GsySiTZkNFy9nwV4g6uQ.m9s8LlF6G";
...
const sendOtp = async ({
  ...
  try {
    ...
    logger.error(
      "Authentica API Error:",
      error.response?.data || error.message,
    );
    // In non-production, still allow registration even if Authentica fails
    if (!isProduction) {
      logger.warn(
        "⚠️  Authentica failed, but allowing in dev mode. Check console for OTP.",
      );
      return {
        data: {
          success: true,
          message: "OTP logged to console (Authentica unavailable)",
        },
      };
    }
    throw error;
