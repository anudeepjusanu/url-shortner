...
const getBioPageById = async (req, res) => {
  try {
    const bioPage = await bioPageService.getBioPageById(
      req.params.id,
      req.user.id,
    );
...
const checkUsernameAvailability = async (req, res) => {
  try {
    const result = await bioPageService.checkUsernameAvailability(
      req.params.username,
    );
...
const getBioPageAnalytics = async (req, res) => {
  try {
    const analytics = await bioPageService.getBioPageAnalytics(
      req.params.id,
      req.user.id,
    );
