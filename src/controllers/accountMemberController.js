...
const getVisibleProjectIds = async (req) => {
  ...
  return projectAccessService.getAdminProjectIds(
    req.user.id,
...
const getOverview = async (req, res) => {
  try {
    ...
    const invitations = await ProjectInvitation.find({
      ...
      "projectRoles.project": { $in: visibleProjectIds },
    }).populate("projectRoles.project", "name");
    ...
    res.json({
      ...
      data: {
        ...
        pendingInvitations: invitations.map((inv) => ({
          id: inv._id,
          email: inv.email,
          // Only the projects THIS caller can see — an Admin invited to see
          // this invitation (because it touches a project they administer)
          // must not learn the role/name of its other, unrelated projects.
          projectRoles: inv.projectRoles
            .filter((pr) =>
              visibleProjectIds.includes(pr.project._id.toString()),
            )
            .map((pr) => ({
              projectId: pr.project._id,
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to load account members" });
  }
};
  ...
  try {
    ...
    const projects = await Promise.all(
      memberships.map(async (membership) => ({
        ...
        projectName: membership.project.name,
        role: membership.role,
        assignableRoles: await projectAccessService.assignableRolesForProject(
          req.user.id,
          membership.project._id,
        ),
      })),
    );
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to load member detail" });
  }
};
...
const inviteMember = async (req, res) => {
  try {
    const { email, projectRoles } = req.body;
    if (
      !email ||
      !Array.isArray(projectRoles) ||
      projectRoles.length === 0 ||
      !projectRoles.every((pr) => pr && pr.projectId && pr.role)
    ) {
      return res.status(400).json({
        success: false,
        message: "email and projectRoles ([{ projectId, role }]) are required",
      });
    }
...
const removeFromAccount = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to remove user from account" });
  }
};
