...
const redirectToOriginalUrl = async (req, res) => {
  ...
  try {
    ...
    try {
      const locationData = await geoLocation.getLocationFromIP(
    ...
    if (error.message === "CONTENT_BLOCKED") {
      return res.redirect(
        `${frontendUrl}/blocked?code=${encodeURIComponent(shortCode)}`,
      );
    }
...
const getPreview = async (req, res) => {
  try {
    ...
    const previewData = await redirectService.generatePreviewPage(
      shortCode,
      requestData,
    );
...
const generateQRCode = async (req, res) => {
  try {
    ...
    const { size, format, fgColor, bgColor, errorCorrection, margin } =
      req.query;
...
const handleDeepLinkRedirect = async (req, res) => {
  ...
  try {
    ...
    const webFallback =
      url.deepLink.webFallbackUrl || app.webFallbackUrl || url.originalUrl;

    // Track the click (non-fatal)
    try {
      let country = "US";
      try {
        const loc = await geoLocation.getLocationFromIP(clientIP);
    ...
    if (platform === "android") {
      ...
      if (app.packageName && app.androidStoreUrl) {
        ...
        const html = deepLinkService.buildAndroidIntentPage(
          intentUrl,
          app.androidStoreUrl,
          app.name,
        );
