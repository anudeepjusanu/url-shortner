...
const checkUrlReachability = async (cleanUrl, timeout = 10000) => {
  ...
  if (
    ...
    return {
      allowed: false,
      message:
        "URL does not exist. The domain could not be found. Please check the URL and try again.",
    };
  }
  ...
  if (errorMsg.includes("ECONNREFUSED")) {
    return {
      allowed: false,
      message:
        "URL is not accessible. The server refused the connection. Please check the URL and try again.",
    };
  }
  ...
  if (status && status >= 400) {
    ...
    if (status === 404)
      return {
        allowed: false,
        message:
          "URL not found (HTTP 404). The page does not exist. Please check the URL.",
      };
...
const createUrl = async (req, res) => {
  try {
    ...
    if (!reachabilityCheck.allowed) {
      return res
        .status(400)
        .json({ success: false, message: reachabilityCheck.message });
    }
    ...
    if (domainId) {
      ...
      if (domainId === "base") {
        ...
        if (
          selectedDomain.owner.toString() !== req.user.id &&
          (!req.user.organization ||
            selectedDomain.organization?.toString() !==
      ...
      selectedDomain = await Domain.getDefaultDomain(
        req.user.id,
        req.user.organization,
      );
    ...
    if (shortCode) {
      ...
      if (isReservedAlias(shortCode)) {
        return res.status(400).json({
          success: false,
          message:
            "This alias is reserved for system use. Please choose a different alias.",
        });
      }
      ...
      if (existingUrl) {
        return res.status(400).json({
          success: false,
          message:
            "This alias already exists (case-insensitive). Please choose a different alias.",
        });
      }
    } else {
      let attempts = 0;
    ...
    const resolvedSource = req.user.apiKey
      ? "api"
      : ["landing", "dashboard", "bulk"].includes(bodySource)
        ? bodySource
        : "dashboard";
    ...
    if (generateQRCode) {
      try {
        ...
        if (populatedUrl.domain) {
          ...
          const protocol = asciiDomain.includes("localhost")
            ? "http://"
            : "https://";
...
const updateUrl = async (req, res) => {
  try {
    ...
    if (updatedUrl.isActive) {
      await cacheSet(
        `url:${updatedUrl.shortCode.toLowerCase()}`,
        updatedUrl,
        config.CACHE_TTL.URL_CACHE,
      );
      if (updatedUrl.customCode) {
        await cacheSet(
          `url:${updatedUrl.customCode.toLowerCase()}`,
          updatedUrl,
          config.CACHE_TTL.URL_CACHE,
        );
      }
...
const getUrlStats = async (req, res) => {
  try {
    ...
    const [totalUrls, activeUrls, totalClicks, topUrls] = await Promise.all([
      Url.countDocuments(filter),
  ...
  try {
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      try {
        ...
        const reachability = reachabilityMap.get(urlValidation.cleanUrl) || {
          allowed: true,
        };
        if (!reachability.allowed) {
          failed.push({
            ...
            customCode: entry.customCode || "",
            title: entry.title || "",
            error: reachability.message || "URL is not reachable",
          });
          continue;
        }
        ...
        if (rawAlias) {
          ...
          if (isReservedAlias(rawAlias)) {
            failed.push({
              ...
              customCode: rawAlias,
              title: entry.title || "",
              error: "Alias is reserved for system use",
            });
            continue;
          }
        ...
        const userTags = Array.isArray(entry.tags)
          ? entry.tags
...
const bulkCreateTemplate = (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    'attachment; filename="bulk-links-template.csv"',
  );
  res.send(csv);
};
...
const getAvailableDomains = async (req, res) => {
  try {
    const domains = await Domain.getUserDomains(
      req.user.id,
      req.user.organization,
    );
    ...
    const domainList = [
      // Base domain (always first and default if user has no custom domains)
      {
        ...
        subdomain: null,
        isDefault:
          activeDomains.length === 0 || !activeDomains.some((d) => d.isDefault),
        shortUrl: baseUrl,
        status: "active",
        isSystemDomain: true,
      },
...
const updateDeepLink = async (req, res) => {
  try {
    const { id } = req.params;
    const { enabled, appRegistration, screen, params, webFallbackUrl } =
      req.body;
    ...
    if (isEnabling && !appRegistration) {
      return res.status(400).json({
        success: false,
        message: "appRegistration is required when enabling deep links",
      });
    }
    ...
    if (isEnabling && appRegistration) {
      ...
      if (!app) {
        return res.status(400).json({
          success: false,
          message: "App registration not found or access denied",
        });
      }
    }
