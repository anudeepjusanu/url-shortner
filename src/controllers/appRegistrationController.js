  ...
  try {
    ...
    if (!name || !webFallbackUrl) {
      return res
        ...
        .json({
          success: false,
          message: "name and webFallbackUrl are required",
        });
    }
    ...
    logger.error("[appReg] create error:", err.message);
    if (err.code === 11000) {
      return res
        .status(409)
        .json({
          success: false,
          message: "An app with this bundle ID or package name already exists",
        });
    }
    return res
      .status(500)
      .json({ success: false, message: "Failed to register app" });
  }
};
...
const list = async (req, res) => {
  try {
    ...
    logger.error("[appReg] list error:", err.message);
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch app registrations" });
  }
};
...
const getOne = async (req, res) => {
  try {
    ...
    if (!app) {
      return res
        .status(404)
        .json({ success: false, message: "App registration not found" });
    }
    ...
    return res.json({
      ...
      data: {
        ...app,
        apiKey: app.apiKey ? `••••••••${app.apiKey.slice(-6)}` : "••••••••",
      },
    });
  } catch (err) {
    logger.error("[appReg] getOne error:", err.message);
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch app registration" });
  }
};
...
const update = async (req, res) => {
  try {
    ...
    return res.json({
      ...
      data: {
        ...obj,
        apiKey: obj.apiKey ? `••••••••${obj.apiKey.slice(-6)}` : "••••••••",
      },
    });
  } catch (err) {
    logger.error("[appReg] update error:", err.message);
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to update app registration" });
  }
};
...
const remove = async (req, res) => {
  try {
    ...
    logger.error("[appReg] delete error:", err.message);
    return res
      .status(500)
      .json({ success: false, message: "Failed to delete app registration" });
  }
};
...
const rotateKey = async (req, res) => {
  try {
    ...
    logger.error("[appReg] rotateKey error:", err.message);
    return res
      .status(500)
      .json({ success: false, message: "Failed to rotate API key" });
  }
};
...
const resolveDeepLink = async (req, res) => {
  try {
    ...
    const callerApp = await AppRegistration.findOne({ apiKey, isActive: true })
      .select("+apiKey")
      .lean();
    if (!callerApp) {
      return res
    ...
    try {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Invalid shortCode" });
    }
    ...
    const url = await Url.findOne({
      ...
      .select("deepLink shortCode")
      .lean();

    if (!url) {
      return res
        .status(404)
        .json({ success: false, message: "Deep link not found" });
    }
    ...
    logger.error("[appReg] resolveDeepLink error:", err.message);
    return res
      .status(500)
      .json({ success: false, message: "Failed to resolve deep link" });
  }
};
