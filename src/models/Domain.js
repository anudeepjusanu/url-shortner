...
const domainSchema = new mongoose.Schema(
  {
    ...
    // Enterprise RBAC: which Project this domain belongs to. Only set when
    // `organization` is set. See claude-implementation-docs/Architecture/rbac-enterprise-project-roles.md
    project: {
      type: mongoose.Schema.Types.ObjectId,
      ref: "Project",
      default: null,
...
domainSchema.virtual("isVerified").get(function () {
  return (
    this.verificationStatus === "verified" && this.verificationRecord.verified
  );
});
...
domainSchema.methods.updateStatistics = async function (
  urlCount = 0,
  clickCount = 0,
) {
  const update = {
...
domainSchema.statics.getDefaultDomain = function (
  userId,
  organizationId = null,
) {
  const filter = {
