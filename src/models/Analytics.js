...
clickSchema.statics.getTopStats = async function (urlId, daysOrRange = 30) {
  ...
  let timestampFilter;
  if (
    typeof daysOrRange === "object" &&
    daysOrRange !== null &&
    ("$gte" in daysOrRange || "$lte" in daysOrRange)
  ) {
    ...
    startDate.setDate(
      startDate.getDate() -
        (typeof daysOrRange === "number" ? daysOrRange : 30),
    );
    timestampFilter = { $gte: startDate };
  }

  logger.info("📊 getTopStats called:", { urlId, timestampFilter });
  ...
  logger.info("📊 getTopStats results:", {
