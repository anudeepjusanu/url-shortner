...
const QRCodes = () => {
  ...
  useEffect(() => {
    ...
    if (links.length > 0) {
      const calculatedTotalScans = links.reduce(
        (sum, link) => sum + (link.qrScanCount || 0),
        0,
      );
      setStats((prevStats) => ({
        ...prevStats,
  ...
  const downloadQRCode = async (linkId, format = "png") => {
    try {
      ...
      if (selectedQR && (selectedQR.id || selectedQR._id) === linkId) {
        setSelectedQR((prev) => ({
          ...prev,
          qrCodeDownloads: (prev.qrCodeDownloads || 0) + 1,
        }));
      }
  ...
  const getShortUrl = (link) => {
    const domain =
      link.domain && !isSystemDomain(link.domain)
        ? link.domain
        : getCurrentDomain();
        ...
        >
          <CardHeader className="pb-3">
            <div className="flex items-start justify-between">
              ...
              <button
                onClick={() => toggleSelection(link.id || link._id)}
                className="ml-2"
              >
            ...
            {link.qrCode ? (
              ...
              >
                <img
                  src={link.qrCode}
                  alt="QR Code"
                  className="w-40 h-40 object-contain transition-transform group-hover/qr:scale-105"
                />
              ...
              <div className="w-40 h-40 bg-slate-100 rounded-lg flex flex-col items-center justify-center text-muted-foreground">
                ...
                <span className="text-xs">
                  {t("qrCodes.emptyState.noQRCodes")}
                </span>
              </div>
            )}
          </CardContent>
          <CardFooter className="flex flex-col gap-3 bg-slate-50/30 p-4">
            <div className="flex items-center justify-between w-full text-xs text-muted-foreground">
              <span className="flex items-center gap-1">
                <Activity className="h-3 w-3" /> {link.qrScanCount || 0}{" "}
                {t("qrCodes.stats.scans")}
              </span>
              <span className="flex items-center gap-1">
                ...
                <div className="flex items-center gap-6 text-sm">
                  ...
                  <div className="text-center">
                    <div className="font-bold text-green-600">
                      {link.qrCodeDownloads || 0}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {t("qrCodes.stats.downloads")}
                    </div>
                  </div>
                </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button onClick={() => setShowGenerateModal(true)}>
            ...
            {t("qrCodes.generate.generateNew")}
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          {
            label: t("qrCodes.stats.totalQRCodes"),
          ...
          {
            label: t("qrCodes.stats.totalScans"),
          ...
          {
            label: t("qrCodes.stats.downloadsToday"),
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              {t("qrCodes.emptyState.title")}
            </h3>
            <p className="text-muted-foreground text-center mb-4">
              {searchQuery
                ? t("qrCodes.emptyState.tryAdjusting")
                : t("qrCodes.emptyState.createFirst")}
            </p>
            {!searchQuery && (
              <Button onClick={() => setShowGenerateModal(true)}>
                <Plus className="mr-2 h-4 w-4" />{" "}
      ...
      <Dialog open={showGenerateModal} onOpenChange={setShowGenerateModal}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("qrCodes.generate.description")}
            </DialogDescription>
          </DialogHeader>
          ...
          {!selectedLink ? (
            <div className="py-4 space-y-4">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedLink(
                    links.find((l) => (l.id || l._id) === e.target.value),
                  )
                }
              >
                <option value="">
                  {t("qrCodes.generate.selectLinkPlaceholder")}
                </option>
                {links.map((l) => (
            ...
            <div className="py-2 space-y-6">
              <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                <p className="text-sm font-medium text-blue-900">
                  {t("qrCodes.generate.generatingFor")}
                </p>
                <p className="text-sm text-blue-700 mt-1">
                  {selectedLink.title || getShortUrl(selectedLink)}
                </p>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setQrOptions({ ...qrOptions, format: e.target.value })
                      }
                    >
                ...
                <div className="flex items-center justify-center p-6 bg-slate-50 rounded-lg border-2 border-dashed">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {t("qrCodes.generate.previewPlaceholder")}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() =>
                selectedLink &&
                generateQRCode(selectedLink.id || selectedLink._id)
              }
      ...
      <Dialog open={showDetailModal} onOpenChange={setShowDetailModal}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("qrCodes.dialogs.details.description")}
            </DialogDescription>
          </DialogHeader>
          {selectedQR && (
            <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    <h3 className="text-sm font-medium text-muted-foreground mb-1">
                      {t("qrCodes.dialogs.details.linkTitle")}
                    </h3>
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-muted-foreground mb-1">
                      {t("qrCodes.dialogs.details.shortUrl")}
                    </h3>
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-mono bg-slate-100 px-3 py-2 rounded flex-1">
                        {getShortUrl(selectedQR)}
                      </p>
                      <Button
                        variant="outline"
                        size="icon"
                        onClick={() => handleCopyUrl(selectedQR)}
                      >
                        {copiedId === (selectedQR._id || selectedQR.id) ? (
                          <Check className="h-4 w-4 text-green-500" />
                        ) : (
                          <Copy className="h-4 w-4" />
                        )}
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-muted-foreground mb-1">
                      {t("qrCodes.dialogs.details.originalUrl")}
                    </h3>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t">
                    <div className="text-center p-3 bg-blue-50 rounded-lg">
                      <div className="text-2xl font-bold text-blue-600">
                        {selectedQR.qrScanCount || 0}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        {t("qrCodes.dialogs.details.totalScans")}
                      </div>
                    </div>
                    <div className="text-center p-3 bg-green-50 rounded-lg">
                      <div className="text-2xl font-bold text-green-600">
                        {selectedQR.qrCodeDownloads || 0}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        {t("qrCodes.dialogs.details.downloads")}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex gap-2 pt-4 border-t">
                <Button
                  ...
                  className="flex-1"
                  onClick={() =>
                    downloadQRCode(selectedQR.id || selectedQR._id, "png")
                  }
                >
                ...
                <Button
                  ...
                  className="flex-1"
                  onClick={() =>
                    downloadQRCode(selectedQR.id || selectedQR._id, "svg")
                  }
                >
                  <FileDown className="mr-2 h-4 w-4" />
                ...
                <Button
                  ...
                  onClick={() =>
                    setShareDialog({
                      isOpen: true,
                      url: selectedQR.customDomain
                        ? `https://${selectedQR.customDomain}/${selectedQR.shortCode}`
                        : `${getCurrentDomain()}/${selectedQR.shortCode}`,
                      title: selectedQR.title || "Check out this QR code",
                    })
                  }
                >
      ...
      <ShareDialog
        ...
        onClose={() => setShareDialog({ isOpen: false, url: "", title: "" })}
        url={shareDialog.url}
